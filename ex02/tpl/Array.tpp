...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/22 23:51:08 by emimenza          #+#    #+#             */
/*   Updated: 2024/07/22 23:51:08 by emimenza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
template <typename T>
Array<T>::Array(void) : mSize(0), mArray(new T[this->mSize])
{
	return;
}
...
template <typename T>
T &Array<T>::operator[](std::size_t const idx) throw(std::out_of_range)
{
	return (this->get(idx));
}

template <typename T>
T const &Array<T>::operator[](std::size_t const idx) const throw(std::out_of_range)
{
	return (this->get(idx));
}

template <typename T>
std::size_t Array<T>::size(void) const throw()
{
	return (this->mSize);
}
...
T &Array<T>::get(std::size_t const idx) const throw(std::out_of_range)
{
	...
	if (idx >= this->mSize)
	{
		o << "std::out_of_range: " << idx << " is out of bounds";
